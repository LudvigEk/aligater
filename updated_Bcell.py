...
def gateBCellDataSet(fcs):
    ...
    gmm=ag.gmm2D(fcs(),"FSC-A","FSC-H",vI=no_clutter,nOfComponents=1)
    singlets=ag.gateGMM(fcs(),"FSC-A","FSC-H",gmm=gmm,sigma=7,vI=no_clutter,plot=False)
    ...
    PBMCstep1=ag.gateThreshold(fcs(),xCol="FSC-A",yCol="SSC-A",vI=singlets, orientation="horisontal",thresh=150000,population="lower",plot=False)
    start=ag.getHighestDensityPoint(fcs(),"FSC-A","SSC-A",vI=PBMCstep1)[1]
    PBMCstep2=ag.shortestPath(fcs(),"FSC-A","SSC-A",maxStep=10,sigma=4,boundaries=[start,150000],vI=PBMCstep1,bins=100, plot=False)
    gmm=ag.gmm2D(fcs(),"FSC-A","SSC-A",2,PBMCstep2,tol=0.01)
    PBMC=ag.gateGMM(fcs(),"FSC-A","SSC-A",gmm,PBMCstep2,sigma=6,plot=False)
...
for fcs in fcsList:
    ...
    if count==0:
        fcs.printStats("/media/ludvig/Project_Storage/GAIM/aligater_output/Full_run_gmm_dijkstra.txt",header=True)
    else:
        fcs.printStats("/media/ludvig/Project_Storage/GAIM/aligater_output/Full_run_gmm_dijkstra.txt",header=False)
    count+=1
